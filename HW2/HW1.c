...
double find_fabs_Max(double *vector, double size) {
...
int input(double *t,int *imax,int *jmax,int *tel,int *le,int *teu,double *dy,double *xsf,double *ysf, double *r, double *w, int *control_case)
{
    ...
    fscanf(input,"%lf %d %d %d %d %d %lf %lf %lf %lf %lf %d",t,imax,jmax,tel,le,teu,dy,xsf,ysf, r, w, control_case); //reading the input
    fclose(input);
    return 0;
}
// Print the output file
int print_output(int ni, int nj, double *x, double *y, int teu, int tel) {
...
int print_residue(int iteration, double *L_x, double *L_y, int ni, int nj) {
    char *output_file_path = "C:\\Users\\roiba\\Documents\\CFD_086376\\HW1\\residue.txt";
    FILE *output;
    if (iteration == 1){
    ...
    double L_x_max = find_fabs_Max(L_x, ni * nj);
    double L_y_max = find_fabs_Max(L_y, ni * nj);
    fprintf(output, "%.15lf %.15lf %d\n", L_x_max, L_y_max, iteration);
...
// Defines the i max vec
void i_max(double *y, double *x, double ni, double ysf, int le, double dy, int imax, int jmax) {
    y[offset2d(imax, 0, ni)] = 0;
    y[offset2d(imax, 1, ni)] = dy;
    for (int j = 2; j <= jmax; j++) {
        y[offset2d(imax, j, ni)] = y[offset2d(imax, j - 1, ni)] + ysf * (y[offset2d(imax, j - 1, ni)] - y[offset2d(imax, j - 2, ni)]);
    }
    for (int j = 1; j <= le; j++) {
        x[offset2d(imax, j, ni)] = x[offset2d(imax, 0, ni)];
    }
}
// Defines the i min vec
void i_min(double *y, double *x, double ni, int le, int dy, int imax, int jmax) {
    for (int j = 1; j <= jmax; j++) {
        y[offset2d(0, j, ni)] = -y[offset2d(imax, j, ni)];
        x[offset2d(0, j, ni)] = x[offset2d(0, j, ni)] + x[offset2d(imax, 0, ni)];
    }
}
// Defines the j max vec
void j_max(double *x,double *y,double  jmax,int  ni, int imax)
{
    int i_line_final = 0;
    double  R = y[offset2d(imax,jmax,ni)];
    double  L = 0.5*PI*R + x[offset2d(imax,jmax,ni)];
    double  delta = L/((ni-1)/2);
    for (int i=0; i<=((ni-1)/2); i++)
    {
    ...
    double  theta = 0.5*PI+atan(x[offset2d(i_line_final,jmax,ni)]/y[offset2d(i_line_final,jmax,ni)]);
    double  d_theta = theta/(25-(i_line_final+1));
    for (int i = ((ni-1)/2);i>=i_line_final;i--)
    {
        x[offset2d(i,jmax,ni)] = R*cos(PI+d_theta*(((ni-1)/2)-i));
        y[offset2d(i,jmax,ni)] = R*sin(PI+d_theta*(((ni-1)/2)-i));
    }
    int k = 1;
    for (int i = ((ni-1)/2)+1;i<=imax;i++)
    {
        x[offset2d(i,jmax,ni)] = x[offset2d(((ni-1)/2)-k,jmax,ni)];
        y[offset2d(i,jmax,ni)] = -y[offset2d(((ni-1)/2)-k,jmax,ni)];
        k++;
    }
}
...
// Calls all the derivatives and puts them into an array
...
//Calculates alpha value for a given i, j
...
// Calculates beta value for a given i, j
...
// Calculates gamma value for a given i, j
...
// Calls and makes an array for alpha, beta, and gamma   
...
// Calls A, B, C and defines the edges
...
int calc_residue(double *L_x,double *L_y,double L_x_max_1,double  L_y_max_1,int  ni,int  nj,int iteration)
{
    double L_x_max = find_fabs_Max(L_x,ni*nj);
    double L_y_max = find_fabs_Max(L_y,ni*nj);
    if (log10(L_x_max/L_x_max_1)      < -6 && log10(L_y_max/L_y_max_1) < -6)
    {
        return 1;
    }
    return 0;
...
int main() {
    double t;int imax;int jmax;int tel;int le;int teu;double dy;double xsf;double ysf;double r;double w;int control_case;double L_x_max_1;double L_y_max_1;
    input(&t,&imax,&jmax,&tel,&le,&teu,&dy,&xsf,&ysf,&r, &w, &control_case);
    ...
    i_max(y, x, ni, ysf, le, dy, imax, jmax);
    i_min(y, x, ni, le, dy, imax, jmax);
    j_max(x, y, jmax, ni, imax);
    ...
    L_operator(alpha, beta, gamma, x, L_x, ni, psi, phi, imax, jmax);
    L_operator(alpha, beta, gamma, y, L_y, ni, psi, phi, imax, jmax);
    L_x_max_1 =  find_fabs_Max(L_x,ni*nj); 
    L_y_max_1 =  find_fabs_Max(L_y,ni*nj); 
    
    int iteration = 1;
    while (calc_residue(L_x,L_y,L_x_max_1,L_y_max_1,ni,nj,iteration) != 1)
    {
       ...
       //print_residue(iteration, L_x, L_y, ni, nj );
       iteration ++;
    }
    print_output(ni, nj, x, y, teu, tel);
